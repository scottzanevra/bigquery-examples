SELECT max(create_time.timestamp) as max_timestamp FROM `<your_project_id>.data_dlp_output.san_francisco_311_service_requests`

SELECT max(finding_id) as max_container_version FROM `<your_project_id>.data_dlp_output.san_francisco_311_service_requests`

SELECT location.content_locations as location_content FROM `<your_project_id>.data_dlp_output.san_francisco_311_service_requests`, UNNEST(location.content_locations) AS locations


SELECT
  max(loc.container_version) as max_container_version
FROM
  `<your_project_id>.data_dlp_output.san_francisco_311_service_requests`,
...
FROM `<your_project_id>.data_dlp_output.san_francisco_311_service_requests`,
UNNEST(location.content_locations) as loc
